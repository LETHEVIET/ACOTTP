...
public class Ants {
    ...
    static double HEURISTIC(int m, int n) {
        return (1.0 / ((double) TTP.instance.distance[m][n] + 0.1));            // TTP: maybe no problem, since this is just a heuristic
    }
    ...
    static void allocate_ants()
        ...
        for (i = 0; i < n_ants; i++) {
            ...
            ant[i].tour = new int[TTP.n + 1];
            ant[i].visited = new boolean[TTP.n];
        }
        best_so_far_ant = new ant_struct();

        best_so_far_ant.tour = new int[TTP.n + 1];
        best_so_far_ant.visited = new boolean[TTP.n];

        restart_best_ant = new ant_struct();

        restart_best_ant.tour = new int[TTP.n + 1];
        restart_best_ant.visited = new boolean[TTP.n];
    ...
    static void evaporation_nn_list()
        ...
        for (i = 0; i < TTP.n; i++) {
            for (j = 0; j < nn_ants; j++) {
                help_city = TTP.instance.nn_list[i][j];
    ...
    static void place_ant(ant_struct a, int step)
        ...
        rnd = (int) (Utilities.ran01(Utilities.seed) * (double) TTP.n); /* random number between 0 .. n-1 */
    ...
    static void choose_best_next(ant_struct a, int phase)
        ...
        assert (0 <= next_city && next_city < TTP.n);
        assert (value_best > 0.0);
        assert (!a.visited[next_city]);
    ...
    static void neighbour_choose_best_next(ant_struct a, int phase)
        ...
        assert (phase > 0 && phase < TTP.n);
        current_city = a.tour[phase - 1];
        assert (0 <= current_city && current_city < TTP.n);
        value_best = -1.; /* values in total matix are always >= 0.0 */
        for (i = 0; i < nn_ants; i++) {
            help_city = TTP.instance.nn_list[current_city][i];
    ...
    static void choose_closest_next(ant_struct a, int phase)
        ...
        for (city = 0; city < TTP.n; city++) {
            if (a.visited[city])
                ...
                if (TTP.instance.distance[current_city][city] < min_distance) { // TTP: maybe no problem, since this is just about the nearest neighbour
                    next_city = city;
                    min_distance = TTP.instance.distance[current_city][city];
                }
            }
        }
        assert (0 <= next_city && next_city < TTP.n);
    ...
    static void neighbour_choose_and_move_to_next(ant_struct a, int phase)
        ...
        assert (current_city >= 0 && current_city < TTP.n);
        for (i = 0; i < nn_ants; i++) {
            if (a.visited[TTP.instance.nn_list[current_city][i]])
                prob_ptr[i] = 0.0; /* city already visited */
            else {
                assert (TTP.instance.nn_list[current_city][i] >= 0 && TTP.instance.nn_list[current_city][i] < TTP.n);
                prob_ptr[i] = total[current_city][TTP.instance.nn_list[current_city][i]];
                sum_prob += prob_ptr[i];
            }
        }
        ...
        if (sum_prob <= 0.0) {
            ...
            help = TTP.instance.nn_list[current_city][i];
            assert (help >= 0 && help < TTP.n);
            assert (!a.visited[help]);
