...
public class InOut {
    ...
    private static final Map<String, BufferedWriter> writer = new HashMap<String, BufferedWriter>();
    ...
    static TTP.point[] read_etsp(String tsp_file_name) throws IOException
        /*
        ...
        Reader reader = new InputStreamReader(new FileInputStream(tsp_file_name), StandardCharsets.UTF_8);
...
//            System.out.println(">>> "+line);
            ...
            if (!found_coord_section) {
                if (line.startsWith("PROBLEM NAME")) {
                    TTP.instance.name = line.split(":")[1].trim();
                } else if (line.startsWith("COMMENT")) {
                } else if (line.startsWith("TYPE") && !line.contains("TSP")) {
                    System.err.println("Not a TSP Tsp.instance in TSPLIB format !!");
                    System.exit(1);
                } else if (line.startsWith("DIMENSION")) {
                    TTP.n = Integer.parseInt(line.split(":")[1].trim());
                    TTP.instance.n = TTP.n;
                    nodeptr = new TTP.point[TTP.n];
                    assert (TTP.n > 2 && TTP.n < 6000);
            ...
            TTP.instance.name = p.getFileName().toString();
    ...
    static void write_report()
         ...
         * COMMENTS: none
         */ {
        System.out.println("best " + Ants.best_so_far_ant.fitness + ", iteration: " + iteration + ", time "
                + Timer.elapsed_time());
        if (comp_report != null)
            printToFile(comp_report, "best " + Ants.best_so_far_ant.fitness + "\t iteration " + iteration
    ...
    static void fprintf_parameters(File file) {
        ...
        printToFile(file, "AcoTsp.boosting\t\t " + ACOTTP.boosting);
    }
...
//	optimal = 1;
    ...
    static void population_statistics()
        ...
        if (stat_report != null) {
            printToFile(stat_report, iteration + "\t" + pop_mean + "\t" + pop_stddev + "\t" + (pop_stddev / pop_mean)
                    + "\t" + branching_factor + "\t" + ((branching_factor - 1.) * (double) TTP.n) + "\t" + avg_distance
                    + "\t" + (avg_distance / (double) TTP.n));
        }
    }
    ...
    static double node_branching(double l)
        ...
        num_branches = new double[TTP.n];

        for (m = 0; m < TTP.n; m++) {
            /* determine max, min to calculate the cutoff value */
            min = Ants.pheromone[m][TTP.instance.nn_list[m][1]];
            max = Ants.pheromone[m][TTP.instance.nn_list[m][1]];
            for (i = 1; i < Ants.nn_ants; i++) {
                if (Ants.pheromone[m][TTP.instance.nn_list[m][i]] > max)
                    max = Ants.pheromone[m][TTP.instance.nn_list[m][i]];
                if (Ants.pheromone[m][TTP.instance.nn_list[m][i]] < min)
                    min = Ants.pheromone[m][TTP.instance.nn_list[m][i]];
            }
            cutoff = min + l * (max - min);

            for (i = 0; i < Ants.nn_ants; i++) {
                if (Ants.pheromone[m][TTP.instance.nn_list[m][i]] > cutoff)
                    num_branches[m] += 1.;
            }
        }
        avg = 0.;
        ...
        if (stat_report != null) {
            for (i = 0; i < TTP.n; i++) {
                printToFile(stat_report, Ants.best_so_far_ant.tour[i] + " "
                        + TTP.instance.nodeptr[Ants.best_so_far_ant.tour[i]].x + " "
                        + TTP.instance.nodeptr[Ants.best_so_far_ant.tour[i]].y);
            }
    ...
    static void exit_try(int ntry)
        ...
        System.out.println("Best Solution in try " + ntry + " is " + Ants.best_so_far_ant.fitness);

        if (report != null)
            printToFile(report, "Best: " + Ants.best_so_far_ant.fitness + "\t Iterations: " + found_best
        ...
        best_in_try[ntry] = Ants.best_so_far_ant.fitness;
    ...
    static void exit_program()
        ...
        if (report != null) {
            ...
            if (optimal > 0) {
                printToFile(report, " excess best = " + ((best_tour_length - optimal) / (double) optimal)
                        + ", excess average = " + ((avg_sol_quality - optimal) / (double) optimal) + ","
                        + " excess worst = " + ((worst_tour_length - optimal) / (double) optimal));
            }
        }

        if (comp_report != null)
            printToFile(comp_report, "end problem " + TTP.instance.name);
    ...
    static void init_program(String[] args) {
        ...
        try {
            TTP.instance.nodeptr = read_etsp(name_buf);
        } catch (IOException e) {
        ...
        LocalSearch.nn_ls = Math.min(TTP.n - 1, LocalSearch.nn_ls);
...
//        boolean mkdir = file.mkdirs();
        if (!quiet_flag) {
            Writer w;
            try {
                temp_buffer = solution_dir + "/best." + TTP.instance.name;
                // // TRACE ( System.out.println("%s\n",temp_buffer); )
                report = new File(temp_buffer);
                w = new OutputStreamWriter(new FileOutputStream(temp_buffer), StandardCharsets.UTF_8);
                writer.put(report.getName(), new BufferedWriter(w));

                temp_buffer = solution_dir + "/cmp." + TTP.instance.name;
                // // TRACE ( System.out.println("%s\n",temp_buffer); )
                comp_report = new File(temp_buffer);
                w = new OutputStreamWriter(new FileOutputStream(temp_buffer), StandardCharsets.UTF_8);
                writer.put(comp_report.getName(), new BufferedWriter(w));

                temp_buffer = solution_dir + "/stat." + TTP.instance.name;
                // // TRACE ( System.out.println("%s\n",temp_buffer); )
        ...
        TTP.instance.distance = TTP.compute_distances();                        // TTP: no problem, just the distance matrix gets calculated
    ...
    static void printDist()
        ...
        for (i = 0; i < TTP.n; i++) {
            ...
            for (j = 0; j < TTP.n - 1; j++) {
                System.out.println(" " + TTP.instance.distance[i][j]);          // TTP: no problem, only prints the distances
            }
            System.out.println(" " + TTP.instance.distance[i][TTP.n - 1]);      // TTP: (same as above)
            System.out.println("\n");
        }
        System.out.println("\n");
    }
    ...
    static void printTotal()
        ...
        for (i = 0; i < TTP.n; i++) {
            ...
            System.out.println(" " + Ants.total[i][TTP.n - 1]);
            if (Ants.total[i][TTP.n - 1] > 1.0)
                System.out.println("XXXXX\n");
        }
        System.out.println("\n");
    }
    ...
    static void printTour(int[] t)
        ...
        System.out.println("\n");
        System.out.println("Tour length = " + TTP.compute_fitness(t));
    }
    ...
    static void printTourFile(int[] t)
        ...
        printToFile(comp_report, "Tour length " + TTP.compute_fitness(t));
    ...
    static void writelog(ArrayList<Pair<TTPSolution, Long>> logs){
        String filepath = solution_dir + "/log." + TTP.instance.name;
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filepath, false))){
            writer.newLine();
            for (Pair<TTPSolution, Long> log : logs) {
                writer.write(log.y + " " + log.x.objectiveScore + "\n");
                writer.write(log.x.answer());
                writer.write("\n");
            }
        }
        catch(IOException e){
            System.err.print("Could not write file " + filepath + " " + e.getMessage());
            System.exit(1);
        }
    }
