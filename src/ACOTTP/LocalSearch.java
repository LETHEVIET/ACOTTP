...
public class LocalSearch {
    ...
    static void two_opt_first(int[] tour)
        ...
        random_vector = generate_random_permutation(TTP.n);
        ...
        while (improvement_flag) {
            ...
            for (l = 0; l < TTP.n; l++) {
                ...
                pos_c1 = pos[c1];
                s_c1 = tour[pos_c1 + 1];
                radius = TTP.instance.distance[c1][s_c1];

                /* First search for c1's nearest neighbours, use successor of c1 */
                for (h = 0; h < nn_ls; h++) {
                    c2 = TTP.instance.nn_list[c1][h]; /* exchange partner, determine its position */
                    if (radius > TTP.instance.distance[c1][c2]) {
                        s_c2 = tour[pos[c2] + 1];
                        gain = -radius + TTP.instance.distance[c1][c2] + TTP.instance.distance[s_c1][s_c2]
                                - TTP.instance.distance[c2][s_c2];
                        if (gain < 0) {
                            h1 = c1;
                            h2 = s_c1;
                            h3 = c2;
                ...
                if (gotoExchange) {
                    ...
                    if (pos_c1 > 0)
                        ...
                        p_c1 = tour[TTP.n - 1];
                    radius = TTP.instance.distance[p_c1][c1];
                    for (h = 0; h < nn_ls; h++) {
                        c2 = TTP.instance.nn_list[c1][h]; /* exchange partner, determine its position */
                        if (radius > TTP.instance.distance[c1][c2]) {
                            pos_c2 = pos[c2];
                            ...
                            if (p_c1 == c2)
                                continue;
                            gain = -radius + TTP.instance.distance[c1][c2] + TTP.instance.distance[p_c1][p_c2]
                                    - TTP.instance.distance[p_c2][c2];
                            if (gain < 0) {
                                h1 = p_c1;
                                h2 = c1;
                                h3 = p_c2;
                ...
                for (h = 0; h < nn_ls; h++) {
                    ...
                    if (radius > TTP.instance.distance[c1][c2]) {
                        ...
                        if (pos_c2 > 0)
                            ...
                            p_c2 = tour[TTP.n - 1];
                        gain = -radius + TTP.instance.distance[c1][c2] + TTP.instance.distance[c2][s_c1]
                                + TTP.instance.distance[p_c2][s_c2] - TTP.instance.distance[c2][s_c2]
                                - TTP.instance.distance[p_c2][c2];
                        if (c2 == s_c1)
                            gain = 0;
                        if (p_c2 == s_c1)
                ...
                if (gotoExchange) {
                    ...
                    for (h = 0; h < nn_ls; h++) {
                        ...
                        if (radius > TTP.instance.distance[c1][c2]) {
                            ...
                            s_c2 = tour[pos[c2] + 1];
                            gain = -radius + TTP.instance.distance[c2][c1] + TTP.instance.distance[p_c1][c2]
                                    + TTP.instance.distance[p_c2][s_c2] - TTP.instance.distance[c2][s_c2]
                                    - TTP.instance.distance[p_c2][c2];
                            if (p_c1 == c2)
                                gain = 0;
    ...
    static void three_opt_first(int[] tour)
        ...
        pos = new int[TTP.n];
        dlb = new boolean[TTP.n];
        h_tour = new int[TTP.n];
        hh_tour = new int[TTP.n];
        ...
        while (improvement_flag) {
            ...
            for (l = 0; l < TTP.n; l++) {
                ...
                while (h < nn_ls) {
                    ...
                    c2 = TTP.instance.nn_list[c1][h]; /* second city, determine its position */
                    pos_c2 = pos[c2];
                    s_c2 = tour[pos_c2 + 1];
                    if (pos_c2 > 0)
                        p_c2 = tour[pos_c2 - 1];
                    ...
                    radius = TTP.instance.distance[c1][s_c1];
                    add1 = TTP.instance.distance[c1][c2];

                    /* Here a fixed radius neighbour search is performed */
                    if (radius > add1) {
                        decrease_breaks = -radius - TTP.instance.distance[c2][s_c2];
                        diffs = decrease_breaks + add1 + TTP.instance.distance[s_c1][s_c2];
                        diffp = -radius - TTP.instance.distance[c2][p_c2] + TTP.instance.distance[c1][p_c2]
                                + TTP.instance.distance[s_c1][c2];
                    } else
                        break;
                    ...
                    while (g < nn_ls) {
                        ...
                        c3 = TTP.instance.nn_list[s_c1][g];
                        pos_c3 = pos[c3];
                        s_c3 = tour[pos_c3 + 1];
                        if (pos_c3 > 0)
                            p_c3 = tour[pos_c3 - 1];
                        ...
                        if (c3 == c1) {
                            ...
                            if (decrease_breaks + add1 < add2) {
                                ...
                                if (between) {
                                    ...
                                    gain = decrease_breaks - TTP.instance.distance[c3][p_c3] + add1 + add2
                                            + TTP.instance.distance[p_c3][s_c2];
                                    ...
                                    gain = decrease_breaks - TTP.instance.distance[c3][s_c3] + add1 + add2
                                            + TTP.instance.distance[s_c2][s_c3];
                                    ...
                                    gain = -radius - TTP.instance.distance[p_c2][c2] - TTP.instance.distance[p_c3][c3]
                                            + add1 + add2 + TTP.instance.distance[p_c2][p_c3];
                                    ...
                                    gain = -radius - TTP.instance.distance[p_c2][c2] - TTP.instance.distance[c3][s_c3]
                                            + add1 + add2 + TTP.instance.distance[p_c2][s_c3];
                    ...
                    if (move_flag != 0) {
                        ...
                        if (move_flag == 4) {
                            ...
                            if (pos_c2 > pos_c1)
                                ...
                                n1 = TTP.n - (pos_c1 - pos_c2);
                            if (pos_c3 > pos_c2)
                                n2 = pos_c3 - pos_c2;
                            else
                                n2 = TTP.n - (pos_c2 - pos_c3);
                            if (pos_c1 > pos_c3)
                                n3 = pos_c1 - pos_c3;
                            else
                                n3 = TTP.n - (pos_c3 - pos_c1);
                            ...
                            if (pos_c3 < pos_c2)
                                ...
                                n1 = TTP.n - (pos_c3 - pos_c2);
                            if (pos_c3 > pos_c1)
                                n2 = pos_c3 - pos_c1 + 1;
                            else
                                n2 = TTP.n - (pos_c1 - pos_c3 + 1);
                            if (pos_c2 > pos_c1)
                                n3 = TTP.n - (pos_c2 - pos_c1 + 1);
                            else
                                n3 = pos_c1 - pos_c2 + 1;
