...
public class TTP {
     ...
     */
    public static HashMap<String, TTPSolution> tours = new HashMap<>();
    ...
    static double compute_fitness(int[] t) {
        boolean doTTP = true;
...
//             tlocal = new int[t.length];
            ...
            if (!tours.containsKey(tourConverted)) {
                ...
                tours.put(tourConverted, sol);
            } else {
                if (debugPrint) System.out.println("h");
                result = (tours.get(tourConverted).getObjective()) * -1;
            }
            return result;
        }
    ...
    static void compute_fitness(Ants.ant_struct ant)
        /*
        ...
        int[] tLocal = rotateTour(ant.tour);
        // check against hashmaps
        String tourConverted = Arrays.toString(tLocal);
        if (!tours.containsKey(tourConverted)) {
            // if not contained: compute.
            ant.sol = ppGreedyRT3(currentTTPInstance,
                    Integer.MAX_VALUE,
                    tLocal,
                    5,
                    2.5,
            ...
            currentTTPInstance.evaluate(ant.sol, false);
            tours.put(tourConverted, ant.sol);
        } else {
            ant.sol = tours.get(tourConverted);
        }

        ant.fitness = (ant.sol.getObjective()) * -1;
    }
//    public static TreeSet tours = new TreeSet();
    ...
    public static int[] rotateTour(int[] t) {
        ...
        if (t[0] != 0) {
            ...
            if (t.length - 1 - zeroIndex >= 0)
                System.arraycopy(t, zeroIndex, tlocal, 0, t.length - 1 - zeroIndex);
            System.arraycopy(t, 0, tlocal, t.length - zeroIndex - 1, zeroIndex);
